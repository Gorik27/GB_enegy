...
if not args.plot:
    ...
    task = f'{lmp} -in in.relax_fixed -var name {args.name} -var structure_name {structure} -sf omp -pk omp {args.jobs}'
    exitflag = False
    db_flag = False
    db = 0
    ...
    with open(f'../workspace/{args.name}/conf.txt', 'r') as f :
        for line in f:
            if 'relaxed' in line:
                line = f'relaxed {datfile}\n'
                flag=True
                print(line)
                output += line
        if not flag:
            output += f'relaxed {datfile}\n'
