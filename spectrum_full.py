...
for i in range(i0, len(ids)):
    ...
    task = f'mpirun -np {args.np} lmp_intel_cpu_openmpi -in in.seg_minimize_full -var count {i} -var name {args.name} -var structure_name {structure}  -var atom_types {atom_arg} -var id {ids_i_arg} -sf omp -pk omp {args.jobs}'
    exitflag = False
    db_flag = False
    db = 0
